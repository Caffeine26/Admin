<template>
    <v-container>
        ...
        <v-form ref="formRef">
            ...
            <v-text-field v-if="promotion.discount_type === 'custom_price'" v-model.number="promotion.custom_price"
                ...
                <v-data-table :headers="productHeaders" :items="filteredProducts" :search="productSearch" dense>
                    ...
                    <template #item.actions="{ item }">
                        <v-btn small color="green" @click="addPromotionItem(item)"
                            :disabled="promotion.promotion_items.some(p => p.product_id === item.id)">
                            Add
                        </v-btn>
                    </template>
                </v-data-table>
            ...
            <!-- Submit -->
            <v-btn color="primary" class="mt-4" @click="updatePromotion">Update Promotion</v-btn>
        </v-form>
    </v-container>
</template>
...
onMounted(async () => {
    ...
    const data = await promotionStore.fetchPromotionById(promotionId)
    if (data) {
        promotion.value = {
            ...data,
            promotion_items: data.promotion_items.map((item: any) => ({ product_id: item.product.id, product: item.product }))
        }
    }
})
...
const updatePromotion = async () => {
    ...
    try {
        ...
        formData.append('discount', promotion.value.discount?.toString() || '0')
        formData.append('custom_price', promotion.value.custom_price?.toString() || '0')
        ...
        await promotionStore.updatePromotion(route.params.id, formData)
