...
<template>
    ...
    <v-list-group no-action>
        ...
        <template v-slot:activator="{ props }">
            <v-list-item v-bind="props" :value="item.title" rounded>
                ...
                <template v-slot:prepend>
                    <!-- <span class="dot"></span> -->
                    <Icon :icon="'solar:' + item.icon" height="18" width="18" :level="level" class="dot mr-3" :class="'text-' + item.BgColor" />
                </template>
                <!---Title  -->
        ...
        <template v-for="(subitem, i) in item.children" :key="i" v-if="item.children">
            ...
            <NavItem class="ps-2" :item="subitem" :level="level + 1" v-else></NavItem>
        </template>
    </v-list-group>
